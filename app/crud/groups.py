        ...
        try:
            ...
            if user.type == UserTypes.principal:
                ...
                if principal.school_id != group.school_id:
                    logger.warning(
                        f"User with id {user.id} tried to create group in school with id {group.school_id}. Not allowed"
                    )
                    raise NotAllowed("Cannot access other schools")
            db.delete(group)
            db.commit()
        ...
        try:
            ...
            if user.type == UserTypes.principal:
                ...
                if principal.school_id != school_id:
                    logger.warning(
                        f"User with id {user.id} tried to create group in school with id {school_id}. Not allowed"
                    )
                    raise NotAllowed("Cannot access other schools")
            return groups
        except Exception as e:
