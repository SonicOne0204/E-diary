...
class HomeworkCRUD:
    ...
    def add_homework(db: Session, user: User, data: HomeworkData):
        try:
            ...
            if group.school_id != data.school_id:
                logger.warning(
                    f"Access denied while creating homework. Group with id {group.id} is assigned to school with id {group.school_id}. Not allowed to give homework"
                )
                raise NotAllowed("Not allowed to give homework to other schools")
            if teacher.school_id != data.school_id:
                logger.warning(
                    f"Access denied while creating homework. Teacher with id {teacher.id} is assigned to school with id {teacher.school_id}. Not allowed to give homework"
                )
                raise NotAllowed("Not allowed to give homework to other schools")
            homework = Homework()
        ...
        try:
            ...
            if user.type == UserTypes.teacher:
                ...
                if teacher.school_id != homework.school_id:
                    logger.warning(
                        f"User with id {user.id} tried to delete homework with id {homework_id}. Not allowed"
                    )
    ...
    @staticmethod
    def update_homework(
        db: Session, user: User, homework_id: int, data: HomeworkDataUpdate
    ):
        try:
            homework: Homework = db.query(Homework).get(homework_id)
            if not homework:
            ...
            if group.school_id != homework.school_id:
                ...
                raise NotAllowed(
                    f"Group with id {group.id} is assigned to school with id {group.school_id}. Not allowed to give homework"
                )
            if teacher.school_id != homework.school_id:
                logger.warning(
                    f"Access denied while creating homework. Teacher with id {teacher.id} is assigned to school with id {teacher.school_id}. Not allowed to give homework"
                ...
                raise NotAllowed(
                    f"Teacher with id {teacher.id} is assinged to school with id {teacher.school_id}. Not allowed to give homework"
                )
            data_dict = data.model_dump(exclude={"due_date"})
            if isinstance(data.due_date, str):
                parsed_dt = datetime.fromisoformat(data.due_date)
