        ...
        for schedule in schedules:
            for student in students:
                ...
                attendance = Attendance(
                    schedule_id=schedule.id, student_id=student.id, created_at=today
                )
                db.add(attendance)
                db.commit()
...
class ScheduleCRUD:
    ...
    def create_schedule(
        ...
        try:
            ...
            if (
                data.school_id != principal.school_id
                and user.type == UserTypes.principal
            ):
                logger.warning(
        ...
        try:
            ...
            if user.type == UserTypes.principal:
                ...
                if schedule.school_id != principal.school_id:
                    logger.warning(
                        f"Principal with id {user.id} cannot delete schedule with id {schedule.id} from school with id {schedule.school_id}"
                    )
                    raise NotAllowed(
    ...
    @staticmethod
    def update_schedule(
        db: Session, user: User, schedule_id: int, data: ScheduleUpdateData
    ):
        try:
            schedule: Schedule = db.query(Schedule).get(schedule_id)
            if user.type == UserTypes.principal:
                ...
                if schedule.school_id != principal.school_id:
                    logger.warning(
                        f"Principal with id {user.id} cannot update from schedule with id {schedule.school_id}"
                    )
                    raise NotAllowed(
