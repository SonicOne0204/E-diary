...
class PrincipalService:
    ...
    def invite_teacher_to_school_id(
        db: Session, user: User, school_id: int, teacher_id: int
    ) -> Invitation:
        ...
        teacher: Teacher = (
            db.query(Teacher).filter(Teacher.id == userside_teacher.id).one_or_none()
        )
        print(type(teacher))
        if teacher == None:
    ...
    @staticmethod
    def invite_student_to_school_id(
        db: Session, user: User, school_id: int, student_id: int
    ) -> Invitation:
        try:
            ...
            student: Student = (
                db.query(Student)
                .filter(Student.id == userside_student.id)
                .one_or_none()
            )
            if student == None:
    ...
    @staticmethod
    def link_student_to_group_id(
        db: Session, user: User, group_id: int, student_id: int
    ):
        try:
            userside_student: User = db.query(User).get(student_id)
            if userside_student == None:
    ...
    @staticmethod
    def link_teacher_to_subject_id(
        db: Session, user: User, teacher_id: int, subject_id: int
    ):
        try:
