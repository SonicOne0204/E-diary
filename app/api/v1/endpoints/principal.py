...
from app.schemas.principals import (
    InviteStudentByIdSchoolModel,
    InviteTeacherByIdSchoolModel,
    AssingStudentByIdGroupModel,
    AssingTeacherByIdSubjectModel,
)
...
def invite_teacher_to_school_id(
    ...
    data: InviteTeacherByIdSchoolModel,
) -> Invitation:
    try:
        invitation = PrincipalService.invite_teacher_to_school_id(
            db=db, school_id=data.school_id, user=user, teacher_id=data.teacher_id
        )
        return invitation
    except NotFound:
        raise HTTPException(
...
def invite_student_to_school_by_id(
    db: Annotated[Session, Depends(get_db)],
    user: Annotated[User, Depends(get_current_user)],
    data: InviteStudentByIdSchoolModel,
) -> Invitation:
    try:
        invitation = PrincipalService.invite_student_to_school_id(
            db=db, user=user, school_id=data.school_id, student_id=data.student_id
        )
        return invitation
    except NotFound:
        raise HTTPException(
...
def assign_student_to_group_by_id(
    db: Annotated[Session, Depends(get_db)],
    user: Annotated[User, Depends(get_current_user)],
    data: AssingStudentByIdGroupModel,
) -> dict:
    try:
        PrincipalService.link_student_to_group_id(
            db=db, user=user, group_id=data.group_id, student_id=data.student_id
        )
        return {
...
def assign_student_to_subject_by_id(
    db: Annotated[Session, Depends(get_db)],
    user: Annotated[User, Depends(get_current_user)],
    data: AssingTeacherByIdSubjectModel,
) -> dict:
    try:
        PrincipalService.link_teacher_to_subject_id(
            db=db, user=user, teacher_id=data.teacher_id, subject_id=data.subject_id
        )
        return {
