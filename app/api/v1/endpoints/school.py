...
def get_school(db: Annotated[Session, Depends(get_db)], user: Annotated[User, Depends(get_current_user)], school_name: str | None = None, school_id: int | None = None) -> School:
    try:
...
@school_router.patch('/{school_id}', response_model=SchoolUpdateOut, dependencies=[Depends(check_role(UserTypes.admin))])
def update_school_data(db: Annotated[Session, Depends(get_db)], school_id: Annotated[int, Path()], data: SchoolUpdate) -> School:
    try:
...
@school_router.delete('/{school_id}', response_model=bool, dependencies=[Depends(check_role([UserTypes.admin, UserTypes.principal]))])
def delete_school(db: Annotated[Session, Depends(get_db)], user: Annotated[User, Depends(get_current_user)], school_id: Annotated[int, Path()]) -> None:
    try:
...
def get_schools(db: Annotated[Session, Depends(get_db)], country: str | None = None, is_active: bool | None = None) -> list[School]:
    try:
