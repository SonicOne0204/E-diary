...
@groups_router.post('/', response_model=GroupDataOut)
def add_group(db: Annotated[Session, Depends(get_db)], user: Annotated[User, Depends(get_current_user)] , data: GroupData):
    try:
...
def get_groups(db: Annotated[Session, Depends(get_db)], user: Annotated[User, Depends(get_current_user)] , school_id: int):
    try:
...
def get_group(db: Annotated[Session, Depends(get_db)], user: Annotated[User, Depends(get_current_user)] , group_id: int):
    try:
...
def delete_group(db: Annotated[Session, Depends(get_db)], user: Annotated[User, Depends(get_current_user)] , group_id: int):
    try:
...
def update_group(db: Annotated[Session, Depends(get_db)], user: Annotated[User, Depends(get_current_user)] , group_id: int, data: GroupData):
    try:
