...
def create_wrong_school(db: Session):
    school = School(name='wrong school', short_name='ws', country='Wrongland', address='Wrong street -1', grade_system=GradeSystems.five_num_sys)
    db.add(school)
    db.commit()
    return school
...
def create_teacher_wrong_school(db: Session):
    ...
    if exists:
        logger.info(f'Teacher already exists. Deleted old teacher and created new one. Teacher: {exists}')
        db.delete(exists)
        db.commit()
    user = Teacher(
        username='teacher', 
