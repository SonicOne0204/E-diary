...
@pytest.fixture()
def create_schedule(test_get_db: Session, create_school, create_group, create_subject, create_teacher_fixture):
    db = test_get_db
    schedule = Schedule(
        group_id = create_group.id, 
        school_id = create_school.id, 
        subject_id = create_subject.id, 
        teacher_id=create_teacher_fixture.id, day_of_week=Week.monday, 
